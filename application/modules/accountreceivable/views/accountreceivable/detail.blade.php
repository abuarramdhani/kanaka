...
<div class="page-content">
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h3 class="page-title"> {{lang('accountreceivable')}} - {{ ucwords($customer->name) }} </h3>
    <!-- END PAGE TITLE-->
    <!-- END PAGE HEADERs-->
    <div class="row">
        <div class="col-md-12">
            ...
            <div id="table-wrapper" class="portlet light bordered">
                <div class="portlet-title">
                    ...
                    <div class="tools">
                        <button onclick="return window.location='{{ base_url() }}reports/accountreceivable'" class="btn btn-default btn-sm">
                            <i class="fa fa-chevron-left"></i> {{ lang('back') }}
                        </button>

                        @if($print_limited_access == 1 || $print_unlimited_access == 1)
                            <button onClick="return window.open('{{base_url()}}reports/accountreceivable/pdf_detail/{{ $customer_id }}')" class="btn btn-danger btn-sm">
                                <i class="fa fa-file-pdf-o"></i> {{ lang('print_pdf') }}
                            </button>
                            <button onClick="return window.open('{{base_url()}}reports/accountreceivable/excel_detail/{{ $customer_id }}')" class="btn btn-success btn-sm">
                ...
                <div class="portlet-body">
                    <table id="table-accountreceivable-detail" class="table table-striped table-bordered table-hover dt-responsive" width="100%" >
                        <thead>
                            <tr>
...
<script type="text/javascript">
    ...
    var oTable =$('#table-accountreceivable-detail').dataTable({
        "responsive": false,
        "bProcessing": true,
        "bServerSide": true,
        "bLengthChange": true,
        "sServerMethod": "GET",
        "sAjaxSource": "{{ base_url() }}accountreceivable/accountreceivables/fetch_data_detail/{{ $customer_id }}",
        "columnDefs": [
