...
class Companyreports extends MX_Controller {
    ...
    function stock(){
        ...
        $data['stocks_dipo_2'] = $this->get_dipo_stock($condition);
        $data['stocks_mitra_2'] = $this->get_mitra_stock($condition);

        $data['stocks_mitra_3'] = $this->get_mitra_stock_2($condition);

        $this->load->blade('companyreport.views.companyreport.stock', $data);
    }

    public function get_mitra_stock_2($condition){
        $user = $this->ion_auth->user()->row();
        $dataSellIn = array();
        $dataSellOut = array();
        $allDataSell = array();
        ...
        $sell_in = $this->db->query("SELECT m_product.id as product_id, m_dipo_partner.id as customer_id, m_product.name as product_name, m_dipo_partner.name as customer_name, net_price_in_ctn_after_tax as nominal, SUM(total_order_in_ctn) as pax
                                        ...
                                        WHERE t_sell_in_company.deleted = 0
                                        AND m_dipo_partner.type = 'partner'
                                        AND (t_sell_in_company.customer_id = $user->dipo_partner_id
                                        OR m_dipo_partner.dipo_id = $user->dipo_partner_id)
                                        $condition
    ...
    public function get_mitra_stock($condition){
        $user = $this->ion_auth->user()->row();
        $dataSellIn = array();
        $dataSellOut = array();
        $allDataSell = array();
        ...
        array_multisort($vc_array_customer_id, SORT_ASC, $vc_array_product_id, SORT_ASC, $vc_array_nominal, SORT_ASC, $vc_array_type, SORT_ASC, $allDataSell);
    ...
    public function get_dipo_stock($condition){
        $user = $this->ion_auth->user()->row();
        $dataSellIn = array();
        $dataSellOut = array();
        $allDataSell = array();
        ...
        if($user->group_id == '3'){
            $sell_out = $this->db->query("SELECT m_product.id as product_id, m_dipo_partner.id as customer_id, m_product.name as product_name, m_dipo_partner.name as customer_name, net_price_in_ctn_after_tax as nominal, SUM(total_order_in_ctn) as pax
                                            ...
                                            INNER JOIN m_dipo_partner ON users.dipo_partner_id = m_dipo_partner.id
                                            WHERE t_sell_out_company.deleted = 0
                                            AND (t_sell_out_company.customer_id = $user->id
                                            OR m_dipo_partner.dipo_id = $user->dipo_partner_id)
                                            $condition
                                            GROUP BY product_id, customer_id, nominal
                                        ;")->result();
