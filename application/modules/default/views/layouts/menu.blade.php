            ...
            <ul class="sub-menu">
                @if(isset($datadetailmenu) && count($datadetailmenu) > 0)
                    @foreach($datadetailmenu as $detailrows)
                        ...
                        $uri_segment_2 = uri_segment(2);
                        if(!empty(uri_segment(2) && !empty(uri_segment(3)))){
                            $uri_segment = uri_segment(2) . '/' . uri_segment(3);
                        }
                        ?>
                        <li class="nav-item {{ ($uri_segment == $detailrows->menu_link || $uri_segment_2 == $detailrows->menu_link) ? 'active' : null }}">
                            <a href="{{ base_url() .  $row->lang . '/'. $detailrows->menu_link }}" class="nav-link">
                                <i class="fa fa-circle-o nav-link" style="right: 193px;top: 8px;"></i>
...
<li class="nav-item {{ (uri_segment(1) == "logs") ? 'active open' : null }}">
    <a href="{{base_url()}}logs" class="nav-link ">
        ...
        <span class="title">{{ lang('history') }}</span>
