...
class Menus extends MX_Controller {
    ...
    public function __construct() {
        ...
        if(!$this->user_profile->get_user_access('Availabled', 'menu')){
    ...
    public function fetch_data() {
        ...
        foreach ($aa_data as $row) {
            ...
            if($this->user_profile->get_user_access('Updated', 'menu')){
                $btn_action .= '<a href="javascript:void()" onclick="viewData(\'' . uri_encrypt($row->menu_id) . '\')" class="btn btn-warning btn-icon-only btn-circle" data-toggle="ajaxModal" title="' . lang('update') . '"><i class="fa fa-edit"></i> </a>';
            }
            if($this->user_profile->get_user_access('Deleted', 'menu')){
                $btn_action .= '<a href="javascript:void()" onclick="deleteData(\'' . uri_encrypt($row->menu_id) . '\')" class="btn btn-danger btn-icon-only btn-circle" title="' . lang('delete') . '"><i class="fa fa-trash-o"></i></a>';
            }

            $row_menu_code = '';
            $row_menu_code .= '<div class="pull-left">';
            $row_menu_code .= '<div id="view_' . uri_encrypt($row->menu_id) . '">' . $row->menu_code . '</div>';
            $row_menu_code .= '<div id="input_' . uri_encrypt($row->menu_id) . '" style="display: none;"><input type="text" class="form-control input-xs" id="menu_code_' . uri_encrypt($row->menu_id) . '" value="' . $row->menu_code . '" onkeyup="angka(this)" style="width: 50%;text-align:center" /></div>';
            $row_menu_code .= '</div>';
            $row_menu_code .= '<div class="pull-right">';
            $row_menu_code .= '<div>';

            if($this->user_profile->get_user_access('Updated', 'menu')){
                $row_menu_code .= '<a onclick="changeOrderMenu(\'' . uri_encrypt($row->menu_id) . '\')" id="link_' . uri_encrypt($row->menu_id) . '"><i class="fa fa-edit"></i> ' . lang('change') . '</a>';
                $row_menu_code .= '<a onclick="saveOrderMenu(\'' . uri_encrypt($row->menu_id) . '\')" id="save_' . uri_encrypt($row->menu_id) . '" style="display: none;"><i class="fa fa-save"></i> ' . lang('save_menu') . '</a>';
            }
    ...
    public function update() {
        if ($this->input->is_ajax_request()) {
            $id = (int) uri_decrypt($this->input->get('menu_id'));
