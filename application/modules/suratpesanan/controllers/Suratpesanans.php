            ...
            if (empty($id_suratpesanan)) {
                if (!empty($get_suratpesanan->name)) {
                    ...
                    $dataProduct = array('name'         => $name,
                                      ...
                                      'suratpesanan_code'    => $suratpesanan_code,
                                      'description'     => $description,
                                      'feature'         => $feature,
                                      'barcode_suratpesanan' => $barcode_suratpesanan,
                    ...
                    $save       = $this->db->insert('t_sp', $dataProduct);
                    $id_suratpesanan = $this->db->insert_id();
                    ...
                    if ($save) {
                        ...
                        $message = "Add " . strtolower(lang('suratpesanan')) . " " . $name . " succesfully by " . $user->full_name;
    ...
    public function deleteImage() {
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($model)) {
                ...
                $message = "Delete " . strtolower(lang('suratpesanan')) . " " .  $model->image . " succesfully by " . $user->full_name;
                $this->activity_log->create($user->id, NULL, json_encode($data_notif), NULL, $message, 'D', 4);
                $status = array('status' => 'success');
            } else {
                $status = array('status' => 'error');
